...
def display(
    table_name_products: str, min: int, max: int, orderBy: str = "name"
) -> List:
    out = []
    query = (
...
def get_all_categories(args: dict = {}, fields: str = "id, id_API"):
...
def _initialize_categories_ids(self):
    """
    Returns a dictionary that contains all Categories. `id_api: id`.
    """
    res = get_all_categories()
    return {e[1]: e[0] for e in res[1]}

def get_category_id(id_api: str) -> Tuple[bool, int]:
...
def write_in_db(categories: List[category]):
